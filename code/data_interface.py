...
class MyDataModule(pl.LightningDataModule):
    def __init__(self, args):
        ...
        self.train_data, self.valid_data, self.test_data, self.inf = self.load_meta_data(data_name, zone)
        
        
    def setup(self, stage):
        if stage == 'fit':
            self.train = MyDataset(self.train_data, self.inf, self.seq_len, self.pre_len)  # , mode='train'
            self.valid = MyDataset(self.valid_data, self.inf, self.seq_len, self.pre_len)
        if stage == 'test':
            self.test = MyDataset(self.test_data, self.inf, self.seq_len, self.pre_len)
        if stage == 'predict':
            self.predict = MyDataset(self.test_data, self.inf, self.seq_len, self.pre_len)
...
class MyDataset(data.Dataset):
    def __init__(self, data, inf, seq_len, pre_len):
        super().__init__()
        self.data = data
        self.inf = inf
    ...
    def __getitem__(self, index: int):    
        prompt = prompts.prompting(self.data, index, self.seq_len, self.pre_len, self.inf)
        answer = prompts.output_template(np.round(np.array(self.data['local_charge'][index+self.seq_len+self.pre_len]), decimals=4))
